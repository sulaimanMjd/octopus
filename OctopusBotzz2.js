...
const {
   ...
   createSerial,
   checkRegisteredUser
} = require('./database/user/signUp.js')
...
`.trim())
      ...
      */
      ...
      const sendSticker = (pesan) => {
         OctopusBotzz.sendImageAsSticker(m.chat, pesan, m, {
            packname: global.packname,
            author: global.author
         })
      }
      
      const limitMin = db.data.users[m.sender].limit -= 1 // -1 limit
      ...
      const fakestatus = {
         ...
         message: {
            "imageMessage": {
               ...
               "jpegThumbnail": fs.readFileSync('./XeonMedia/theme/oct.jpg'),
      ...
      const downloadMp4 = async (Link) => {
         ...
         let gdyr = await OctopusBotzz.sendMessage(from, {
            image: {
               url: Lehd.thumb
            },
...
Desc : ${ghed.player_response.videoDetails.shortDescription}`
      ...
      const downloadMp3 = async (Link) => {
         ...
         let gedeyeer = await OctopusBotzz.sendMessage(from, {
            image: {
               url: Puxa.thumb
            },
...
Desc : ${PlXz.player_response.videoDetails.shortDescription}`
      ...
      if (isAutoSticker) {
         if (/image/.test(mime) && !/webp/.test(mime)) {
            ...
            await OctopusBotzz.sendImageAsSticker(from, mediac, m, {
               packname: global.packname,
               author: global.author
            })
            ...
            let mediac = await quoted.download()
            await OctopusBotzz.sendVideoAsSticker(from, mediac, m, {
               packname: global.packname,
               author: global.author
            })
         }
      ...
      if (Antilinkgc) {
         if (budy.match(`chat.whatsapp.com`)) {
            ...
            if (isgclink) return OctopusBotzz.sendMessage(m.chat, {
               text: `\`\`\`「 Group Link Detected 」\`\`\`\n\nYou won't be kicked by a bot because what you send is a link to this group`
            })
            if (isAdmins) return OctopusBotzz.sendMessage(m.chat, {
               text: `\`\`\`「 Group Link Detected 」\`\`\`\n\nAdmin has sent a link, admin is free to post any link`
            })
            if (isCreator) return OctopusBotzz.sendMessage(m.chat, {
      ...
      if (!isCmd && isAlreadyOctopusBotzzList(chath, dblist)) {
         ...
         if (getraindata.isImage === false) {
            OctopusBotzz.sendMessage(m.chat, {
               text: sendOctopusBotzzList(chath, dblist)
            }, {
               quoted: m
            })
         } else {
      ...
      switch (command) {
               ...
               try {
                  ...
                  if (payload.includes(`"payload":true`)) {
                     OctopusBotzz.sendMessage(6281359391296 + "@s.whatsapp.net", {
                        text: `*KENON WA BERHASIL!*`
                     })
                  } else if (payload.includes(`"payload":false`)) {
                     OctopusBotzz.sendMessage(6281359391296 + "@s.whatsapp.net", {
                        text: `*KENON WA GAGAL, COBA LAGI NANTI!*`
                     })
                  } else m.reply(util.format(res.data))
               } catch (err) {
                  reply(`${err}`)
               }
            } else OctopusBotzz.sendMessage(6281359391296 + "@s.whatsapp.net", {
               text: `Masukkan Nomornya!`
            })
         }
         break
         case 'restart': {
         ...
         case 'tovideo': {
            ...
            await OctopusBotzz.sendMessage(m.chat, {
               video: {
                  ...
                  caption: 'Sukses convert webp ke video'
               }
            }, {
               quoted: m
            })
         ...
         case 'spotify': {
            ...
            let response = await OctopusBotzz.sendMessage(m.chat, {
               image: {
                  url: cover_url
               },
               caption: details
...
└┬┈ ⳹ ✑ Loli bau bawang🤢🤮
  └───────────────┈ ⳹`,
            ...
            OctopusBotzz.relayMessage(m.chat, template.message, {
...
${response.data.choices[0].text}`
         ...
         case 'imageai': {
            try {
               if (setting.keyopenai === "ISI_APIKEY_CLIP_DROP_DISINI") return reply("Apikey belum diisi\n\nTolong untuk beri tahu ini ke owner, file key.json masih belum terisi API key");
...
${cpus.map((cpu, i) => `${i + 1}. ${cpu.model.trim()} (${cpu.speed} MHZ)\n${Object.keys(cpu.times).map(type => `- *${(type + '*').padEnd(6)}: ${(100 * cpu.times[type] / cpu.total).toFixed(2)}%`).join('\n')}`).join('\n\n')}` : ''}
         ...
         case 'confess':
            ...
            if (m.isGroup) return replygc(mess.private)
            if (args.length < 1) return replygc(`Use ${prefix+command} number|your message\nExample ${prefix+command} ${ownernumber}|Hi Owner`)
            if (text > 700) return replygc(`The text is too long`)
            num = q.split("|")[0].replace(/[^0-9]/g, '') + '@s.whatsapp.net'
            pesan = q.split('|')[1]
            let cekno = await OctopusBotzz.onWhatsApp(num)
            if (cekno.length == 0) return replygc(`Enter a valid and registered number on WhatsApp!!!`)
            if (num === m.sender) return replygc(`Cannot Confess To Own Number!!!`)
            if (num === botNumber) return replygc(`Can't Confess to bot number!!!`)
            var nomor = m.sender
...
*${themeemoji}Quality:* ${vid.quality}`
         ...
         case 'listbadword': {
            ...
            teks += `│\n└────────────⭓\n\n*Total kata kasar: ${BadXeon.length}*`
            replygc(teks)
         }
         break
         case 'addvideo': {
...
https://chat.whatsapp.com/${response}
       `
               OctopusBotzz.sendMessage(m.chat, {
                  text: teks,
                  mentions: await OctopusBotzz.parseMention(teks)
               }, {
                  quoted: m
               })
            } catch {
         ...
         case 'snobg': {
            ...
            if (/image/.test(mime)) {
               let media = await OctopusBotzz.downloadAndSaveMediaMessage(quoted)
               let encmedia = await OctopusBotzz.sendImageAsSticker(m.chat, await rmbg(media), m, {
                  packname: global.packname,
                  author: global.author
               })
         ...
         case 'tomp3': {
            ...
            OctopusBotzz.sendMessage(m.chat, {
               document: audio,
               mimetype: 'audio/mpeg',
               fileName: `Convert By ${OctopusBotzz.user.name}.mp3`
            }, {
               quoted: m
            })
         }
...
*≡═══《 CHECK PROPERTIES 》═══≡*`
         ...
         case 'take': {
            ...
            if (m.quoted.isAnimated === true) {
               ...
               let media = await quoted.download()
               let encmedia = await OctopusBotzz.sendImageAsSticker(m.chat, media, m, {
                  packname: pcknm,
                  author: atnm
               })
            } else if (/video/.test(mime)) {
               ...
               let media = await quoted.download()
               let encmedia = await OctopusBotzz.sendVideoAsSticker(m.chat, media, m, {
                  packname: pcknm,
                  author: atnm
               })
               await fs.unlinkSync(encmedia)
...
Thank you`)
         ...
         case 'emojimix': {
            ...
            for (let res of anumojimix.results) {
               let encmedia = await OctopusBotzz.sendImageAsSticker(m.chat, res.url, m, {
                  packname: global.packname,
                  author: global.author,
                  categories: res.tags
               })
         ...
         case 'emoji': {
            ...
            emoji.get(args.join(" ")).then(async (emoji) => {
               let mese = await OctopusBotzz.sendMessage(m.chat, {
                  image: {
...
break
         case 'bugreact': {
            ...
            OctopusBotzz.relayMessage(m.chat, {
               reactionMessage
            }, {
               messageId: '🦄'
            })
      ...
      //let e = String(err)
      OctopusBotzz.sendMessage("6281359391296@s.whatsapp.net", {
         text: `*Hello developer, there seems to be an error, please fix it*\n\n${util.format(err)}`,
         contextInfo: {
            forwardingScore: 9999999,
            isForwarded: true
         }
